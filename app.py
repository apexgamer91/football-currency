...
def signup():
    if request.method == "POST":
        ...
        try:
            conn.execute("""
                INSERT INTO players (username,password,balance,bank_cash,cash,fc_coin,card_limit,registration_date,profile_pic)
                VALUES (?,?,?,?,?,?,?,datetime('now'),?)
...
def chat():
    ...
    if request.method=="POST":
        ...
        conn.execute("INSERT INTO messages (senderID,receiverID,content) VALUES (?,?,?)",
                     (session["playerID"],0,content))  # receiverID=0 means public chat
        conn.commit()
